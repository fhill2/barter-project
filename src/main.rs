...
use barter_integration::channel::{mpsc_unbounded, ChannelTxDroppable, Tx, UnboundedTx};
...
pub struct CustomInstrumentKey;
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
pub struct CustomDataKind;

// pub type CustomEngineEvent =
// EngineEvent<CustomDataKind, ExchangeIndex, AssetIndex, CustomInstrumentKey>;
// pub struct CustomMarketStreamResult(MarketStreamResult<CustomInstrumentKey, CustomDataKind>);
pub type CustomMarketStreamResult = MarketStreamResult<CustomInstrumentKey, CustomDataKind>;

pub type CustomEngineEvent =
    EngineEvent<CustomDataKind, AssetIndex, AssetIndex, CustomInstrumentKey>;

fn mock_stream() -> impl Stream<Item = CustomMarketStreamResult> + use<> {
    let event = MarketEvent {
        time_exchange: Utc.with_ymd_and_hms(2023, 2, 18, 9, 27, 59).unwrap(),
        time_received: Utc.with_ymd_and_hms(2023, 2, 18, 9, 27, 59).unwrap(),
        exchange: ExchangeId::Simulated,
        instrument: CustomInstrumentKey,
        kind: CustomDataKind {},
    };
    let event = Event::Item(Ok(event));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialise Channels
    let (feed_tx, mut feed_rx) = mpsc_unbounded::<CustomMarketStreamResult>();
    ...
    let market_stream = mock_stream();
    // Forward market data events to Engine feed
    tokio::spawn(market_stream.forward_to(feed_tx.clone()));

    let last_exchange_time: DateTime<Utc> = Utc.with_ymd_and_hms(1968, 2, 2, 0, 0, 0).unwrap();
    let clock = HistoricalClock::new(last_exchange_time);
...
// let execution_txs =
//     MultiExchangeTxMap::from_iter(vec![(ExchangeId::Simulated, Some(execution_tx))]);
...
// ) -> (
//     HistoricalClock,
//     impl Stream<Item = MarketStreamEvent<InstrumentIndex, CustomDataKind>> + use<>,
// ) {
//     let data = std::fs::read_to_string(file_path).unwrap();
//     let events =
//         serde_json::from_str::<Vec<MarketStreamResult<InstrumentIndex, CustomDataKind>>>(&data)
//             .unwrap();
//
